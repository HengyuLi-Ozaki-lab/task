...
CONTAINS
  ...
  ! ----------------------------
  ! SAVE RESULTS INTO WAVES IDS for beam tracing
  ! ----------------------------

  SUBROUTINE put_ids_waves_beam

    USE ids_routines,ONLY: &
         imas_open_env,imas_create_env,imas_close,ids_get
    USE ids_schemas,ONLY: &
         ids_waves,ids_ec_launchers,ids_equilibrium
    IMPLICIT NONE
    type(ids_waves):: waves_ids
    ...
    ! OPEN INPUT DATAFILE FROM OFFICIAL ITER DB
    git call imas_open_env('ids' ,131024,1,idx,'public',trim(machine),'3')
