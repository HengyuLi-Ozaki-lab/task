...
\textbf{Unify} the GitHub line (\C{ats-fukuyama}) and the Kyoto main-dev line (\C{bpsi}) into a
...
\begin{tikzpicture}[>={Latex[length=2mm]},
  ...
  \node[box, fill=blue!8]                  (gh) at (0, 1.15) {GitHub backup\\ \texttt{ats-fukuyama}\\ (stale)};
...
\end{tikzpicture}
\end{center}
\textbf{Purpose:} unify the GitHub (\texttt{ats-fukuyama}) and Kyoto (\texttt{bpsi}) source lines into
\emph{one CI-validated, reliable baseline} that different developers branch from for subsequent merges
and new features --- instead of re-reconciling divergent trees each time.
\end{frame}
...
\]
\begin{block}{Finding}
The new \C{libnf} table \C{svnf\_dt} \emph{is} this same fit, tabulated to 2 significant figures
(\C{svnf\_dt} in cm$^3$/s, \C{SIGMAM} in m$^3$/s; agreement is after the cm$^3$/s$\to$m$^3$/s conversion):
they agree to $0.03$--$1.2\%$ at the table points, and $\le 1.2\%$ over the core range
$T=1$--$30$\,keV (dominated by the 2-sig-fig rounding).
\end{block}
$\Rightarrow$ migrating DT to \C{model\_pnf} shifts the DT reactivity $\sim$1\% (hence \C{SNF}, modulo assembly units) for \textbf{zero physics gain};
\C{model\_pnf}'s value is the \textbf{multi-reaction} capability (DD/DHe3/TT/THe3), which is additive.
\end{frame}
...
\begin{enumerate}
\item \textbf{\C{.so}-only tests miss static-binary issues} --- menus/some modules are excluded from
      \C{libXapi.so}; one \C{pl\_view} break only appeared at \emph{link} time.
\item \textbf{Truncated tool output misleads} --- verify against the complete data.
\item \textbf{1e-10 equivalence is too tight for FP-heavy modules across compiler versions}.
\item \textbf{An independent-reviewer pre-push gate earns its keep} --- caught 3 real issues.
\item \textbf{Physics-change baselines need explicit owner approval} + rigorous characterization.
\item \textbf{Verify a new capability adds physics before migrating} --- \C{model\_pnf} DT $=$ the
      legacy fit ($\sim$1\% rounding), so DT stayed legacy; \C{model\_pnf} reserved for multi-reaction.
\end{enumerate}
\end{frame}

\section{Results \& roadmap}
\begin{frame}{Repositories \& current result}
\footnotesize
\begin{tabular}{@{}p{40mm}p{32mm}p{36mm}@{}}
\toprule
Repo & Role & State \\
\midrule
\texttt{ats-fukuyama/task} & GitHub backup (stale) & --- \\
\texttt{bpsi\dots/task} & Kyoto main dev & base \texttt{fa9dd493} \\
\texttt{HengyuLi-Ozaki-lab/task} & \textbf{unified fork (delivered)} & \texttt{kyoshimi-develop} @ \texttt{d1f7f9e7} \\
\texttt{\dots/task-merge} & merge/test sandbox & PR \#1 merged \\
\bottomrule
\end{tabular}
\vspace{0.8em}
\begin{center}
\begin{tikzpicture}[bg/.style={draw, rounded corners, align=center, font=\scriptsize,
  ...
  \node[bg, fill=blue!14]   at (5.4,0) {\textbf{delivered P0}\\ \texttt{d1f7f9e7}};
...
\textbf{P0 --- delivered}
\begin{itemize}
\item merge \C{0254aa91} $\to$ \C{d1f7f9e7}
\item CI \good{}: 736 passed
\item on \C{myfork/kyoshimi-develop}; PR \#1 merged
\end{itemize}
\column{0.5\textwidth}
\textbf{P1 --- in progress}
\begin{itemize}
\item Task 1 complete (Option A decided)
\item strategy locked + justified
\item Tasks 2--10: defined mechanical port + 1e-10 validation
\end{itemize}
\end{columns}
\vspace{1em}
