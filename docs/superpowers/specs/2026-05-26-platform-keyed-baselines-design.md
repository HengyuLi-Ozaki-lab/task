# Platform-keyed equivalence baselines (design) — **SUPERSEDED**

> **⚠️ SUPERSEDED** by `docs/superpowers/specs/2026-05-26-linux-canonical-equiv-policy-design.md`.
>
> This spec was reviewed (Codex 3 rounds SHIP IT) but its implementation revealed two structural blockers (Codex 2026-05-26 execution-blocker review):
> 1. **6 of 7 modules cannot build standalone Fortran binaries on macOS** (only `tot` has graphics-free static stubs).
> 2. **6-8 of 20 baseline cases have no Python fixture** (`eq_jt60`, `fp_jt60`, `ti_min`, `ti_w`, `tr_m0904`, `wrx_jt60`, plus tot name-mismatch) — they were generated by the Linux-only standalone-binary regen workflow and are dead baselines.
>
> Fixing both gaps requires Phase-L-scale Fortran work + new Python fixtures (multi-day scope creep beyond #213's intent). The pivot to Linux-canonical policy (skip on non-Linux with documented reason + Linux CI is canonical) is implemented in the successor spec. This file is kept as **design history** so future attempts to revisit platform-keyed baselines have the rejected design + the empirical reasons it was rejected.
