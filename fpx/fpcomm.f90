      ...
      contains
        ...
        subroutine fp_allocate
          ...
          IF(MODEL_DISRUPT.ne.0)THEN
             allocate(ER_drei(NRMAX),ER_crit(NRMAX))
             allocate(Rconnor(NRMAX), lnl_gl(NRMAX),RP_crit(NRMAX))
             allocate(RN_disrupt(NRMAX),RN_runaway(NRMAX))
             allocate(RN_drei(NRMAX),RN_runaway_M(NRMAX))
             allocate(Rj_ohm(NRMAX),RJ_runaway(NRMAX))
             allocate(RJ_bs(NRMAX),R_djdt(NRMAX))
             allocate(RJ_bsm(NRSTART:NREND))
             allocate(previous_rate(nrstart:nrend))
             allocate(previous_rate_p(nrstart:nrend))
...
!         NLMAXM= 8 ! this is for analysis without bounce average: 3x3-1 
!         NLMAXM=11 ! this is for analysis without radial transport: 3x4-1
!         NLMAXM=15 ! this is for analysis with a simple radial transport:4x4-1
        ...
        subroutine fp_deallocate
          ...
          END IF
          IF(MODEL_DISRUPT.ne.0)THEN
             deallocate(ER_drei,ER_crit)
             deallocate(Rconnor,lnl_gl,RP_crit)
             deallocate(RN_disrupt,RN_runaway)
             deallocate(RN_drei,RN_runaway_M)
             deallocate(Rj_ohm,RJ_runaway)
             deallocate(RJ_bs,R_djdt)
             deallocate(RJ_bsm)
             deallocate(previous_rate)
             deallocate(previous_rate_p)
             deallocate(previous_rate_G,previous_rate_p_G)
             deallocate(SIGMA_SPP,SIGMA_SPM)
             deallocate(RE_PITCH)
             deallocate(RT_quench,RT_quench_f)          
             deallocate(POST_LNLAM_f)
             deallocate(POST_LNLAM)
