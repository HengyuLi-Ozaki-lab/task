...
CONTAINS
  ...
  FUNCTION pdkf_eul_org(xi,eta,rnu,np_tau)
    ! xi=(x*omega)/vt
    ! eta=vt/(L*omega)
    ! rnu=(omega-n*omegac)/omega
    IMPLICIT NONE
    ...
    RKY=0.D0
    n1=np_tau
    CALL wi_eul(X,ALFA,BETA0,RKY,CS,n1,5,L)
    pdkf_eul_org=CS
    RETURN
  END FUNCTION pdkf_eul_org
  ...
  !  rky=k_y c/omega
  !
  !  G3=X/BETA       =x_real omega/vte
  !  G4=0.5*ALFA*BETA=0.5*vte/L omega
  !  G5=rky*beta     =k_y vte/omega

  SUBROUTINE wi_eul(X,ALFA,BETA0,RKY,CS,n1_,M,L)
