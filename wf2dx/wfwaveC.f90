...
MODULE wfwaveC
  ...
  PUBLIC wf_wave_cold
  PUBLIC wf_wpre_cold
  PUBLIC wf_dtensr_cold
  PUBLIC wf_cmcalc_cold

CONTAINS

  SUBROUTINE wf_wave_cold
...
9000 CONTINUE
    ...
    RETURN
  END SUBROUTINE wf_wave_cold

!     ********** WF WAVE PREPARATION **********

  SUBROUTINE wf_wpre_cold(IERR)
    ...
    RETURN
  END SUBROUTINE wf_wpre_cold

!     ****** DIELECTRIC TENSOR ******

  SUBROUTINE wf_dtensr_cold(NE,DTENS)
...
8000 continue
  ...
  IF(nrank.EQ.0) write(6,*) 'wf_cvsolv: sort started (cold)'
  CALL qsort_lc(NSEQ,CEQP)
  IF(nrank.EQ.0) write(6,*) 'wf_cvsolv: reduction started (cold)'
  NNZME=1
  DO NNZ=2,NNZMAX
