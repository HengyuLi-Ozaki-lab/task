...
!     ------- Set E+, E- and Ez0 to those outside the plasma ---------
    ...
    IF(ALLOCATED(CEF1)) DEALLOCATE(CEF1)
    IF(ALLOCATED(CEF2)) DEALLOCATE(CEF2)
    IF(ALLOCATED(CEFM1)) DEALLOCATE(CEFM1)
    IF(ALLOCATED(CEFM2)) DEALLOCATE(CEFM2)
    IF(ALLOCATED(CEFLDR)) DEALLOCATE(CEFLDR)
    IF(ALLOCATED(CRMARHF)) DEALLOCATE(CRMARHF)
    IF(ALLOCATED(CEFLDM)) DEALLOCATE(CEFLDM)
...
!     ****** CALCULATE MAGNETIC FIELD ******
  ...
  SUBROUTINE wm_bfield
    ...
    IF(ALLOCATED(CBF1)) DEALLOCATE(CBF1)
    IF(ALLOCATED(CBF2)) DEALLOCATE(CBF2)
...
!     ****** CALCULATE ANTENNA IMPEDANCE ******
  ...
  SUBROUTINE wm_pwrant
    ...
    IF(ALLOCATED(CFVP)) DEALLOCATE(CFVP)
...
!     ****** CALCULATE ABSORBED POWER ******
  ...
  SUBROUTINE wm_pabs
    ...
    COMPLEX(rkind):: CPC21,CPP21,CPC22,CPC23
    COMPLEX(rkind):: CPC31,CPP31,CPC32,CPC33
    ...
    DO NS=1,NSMAX
       ...
       DO NR=nr_start,MIN(nr_end,nrmax-1)
          ...
          IF(MDLWMX.EQ.0) THEN
             CALL wm_setf0(NR+1,NS)
          ELSE
             CALL wm_setf2(NR+1,NS)
          END IF
          DO NDX=1,NDSIZ
             DO KDX=1,KDSIZ_F
                DO MDX=1,MDSIZ
...
!     +++++ CALCULATE DRIVEN CURRENT IN REAL SPACE +++++
    ...
    DO n=0,nsize-1
       ...
       nr2=MIN(nr_end_nrank(n),nrmax-1)+1
...
!       IF(NS.EQ.1) THEN
!       DO nr=1,nrmax
!          WRITE(21+nrank,'(A,2I8,4ES12.4)') &
!               'CPABS:',nrank,nr,CPABSK(1,1,nr,1),CEFLDK(1,1,1,NR)
!       END DO
!       END IF
    ...
    IF(ALLOCATED(CPF1)) DEALLOCATE(CPF1)
    IF(ALLOCATED(CPF2)) DEALLOCATE(CPF2)
    IF(ALLOCATED(rdata)) DEALLOCATE(rdata)
    IF(ALLOCATED(rtot)) DEALLOCATE(rtot)
    IF(ALLOCATED(cdata)) DEALLOCATE(cdata)
    IF(ALLOCATED(ctot)) DEALLOCATE(ctot)
