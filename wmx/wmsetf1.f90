...
!     ----- Fourier decompose A, B, C -----
    ...
    IF(ALLOCATED(CEP0)) DEALLOCATE(CEP0)
    IF(ALLOCATED(CRA)) DEALLOCATE(CRA)
    IF(ALLOCATED(CFA)) DEALLOCATE(CFA)
    IF(ALLOCATED(CRB)) DEALLOCATE(CRB)
    IF(ALLOCATED(CFB)) DEALLOCATE(CFB)
    IF(ALLOCATED(CRC)) DEALLOCATE(CRC)
    IF(ALLOCATED(CFC)) DEALLOCATE(CFC)
    IF(ALLOCATED(CMC)) DEALLOCATE(CMC)
    IF(ALLOCATED(CMF)) DEALLOCATE(CMF)
    IF(ALLOCATED(CS)) DEALLOCATE(CS)
    IF(ALLOCATED(CSUMA)) DEALLOCATE(CSUMA)
    IF(ALLOCATED(CSUMPF)) DEALLOCATE(CSUMPF)
...
!     ****** CALCULATE LOCAL MATRIX ******
  ...
  SUBROUTINE wm_setf1_out(NR1,NS0)
    ...
    INTEGER:: ND,MD,NS1,NS2,NS,NHH,NTH,I,J,K,L,KD,LD,KDX,LDX,NDX,MDX
    INTEGER:: KA,KAX,KB,KBX,NB1,NB2,LA,LAX,LB,LBX,MB1,MB2,LAB,LABX,KAB,KABX
    INTEGER:: LBXK,KBXK,KC,KCX,LC,LCX,NR
    INTEGER:: ILL
...
!     ----- Fourier decompose A, B, C -----
    ...
    DO ND=NDMIN,NDMAX
     ...
     IF(ALLOCATED(CS)) DEALLOCATE(CS)
     IF(ALLOCATED(CSUMA)) DEALLOCATE(CSUMA)
     IF(ALLOCATED(CSUMAD)) DEALLOCATE(CSUMAD)
     IF(ALLOCATED(CSUMPF)) DEALLOCATE(CSUMPF)
     IF(ALLOCATED(CPF)) DEALLOCATE(CPF)
