...
class TestEquivalence(unittest.TestCase):
    ...
    @pytest.mark.xfail(
        strict=True,
        reason=(
            "#222: PROFN2/PROFT2 are plcomm scalars with no CASE entry in "
            "fp/fp_param_registry.f90; the wrapper cannot apply them so the "
            "fp_jt60 baseline (generated by the legacy binary which reads "
            "the namelist directly into plcomm) is unreachable. Profile "
            "fields drift 0.5-51%. Remove this xfail when fp registry "
            "exposes PROFN2/PROFT2 AND baseline is regenerated."
        ),
    )
    def test_jt60(self):
