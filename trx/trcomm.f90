...
! TRSRC
  ...
  REAL(rkind), DIMENSION(:,:),   ALLOCATABLE :: & ! (NRM,NSTM)
       PIN, SSIN, PBCL, PFCL, PRF
  REAL(rkind), DIMENSION(:,:),   ALLOCATABLE :: & ! (NSTM,NRM)
       SPE_NSNR
...
! TRINS
  ...
  SUBROUTINE allocate_trcomm(ierr)
    ...
    ALLOCATE(PBCL(NRMAX,NSTM),PFCL(NRMAX,NSTM),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(SPE_NSNR(NSTM,NRMAX),PRF(NRMAX,NSTM),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
 ...
 900 continue
  ...
  SUBROUTINE DEALLOCATE_TRCOMM
    ...
    DEALLOCATE(SIE,SCX,TSIE,TSCX,PIN,SSIN,PBCL,SPE_NSNR,SPE_NSNPELNR)
    DEALLOCATE(SPSC_NSNR)
  ...
  SUBROUTINE DEALLOCATE_ERR_TRCOMM
    ...
    IF(ALLOCATED(SPE_NSNR ))     DEALLOCATE(SPE_NSNR )
