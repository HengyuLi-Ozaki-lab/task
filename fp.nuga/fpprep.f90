         ...
         DO NR=NRSTARTW,NRENDWM
            IF(NR.ge.1.and.NR.le.NRMAX)THEN
               DO NP=NPSTARTW,NPENDWM
                  ...
                  END DO
!                  IF(FNSP(1,NP,NR,NSA).ne.FNSP(1,NP,NR,NSA))THEN
!                     WRITE(*,'(A,3I5,2E14.6)') "TEST FNSP, ", NP, NR, NSA, RN_TEMP(NR,NS), RT_TEMP(NR,NS)
!                  END IF
               ENDDO
            END IF
         END DO
            ...
            DO NR=NRSTARTW,NRENDWM
               IF(NR.ge.1.and.NR.le.NRMAX)THEN
                  DO NP=NPSTARTW,NPENDWM
                     ...
                     END DO
!                     IF(FNSP_DEL(1,NP,NR,NSA).ne.FNSP_DEL(1,NP,NR,NSA) )THEN
!                        WRITE(*,'(A,3I5,2E14.6)') "TEST FNSP_DEl, ", NP, NR, NSA, &
!                             RN_TEMP(NR,NS), RT_TEMP(NR,NS)
!                     END IF
!                     IF(FNSP_MXWL(1,NP,NR,NSA).ne.FNSP_MXWL(1,NP,NR,NSA) )THEN
!                        WRITE(*,'(A,3I5,2E14.6)') "TEST FNSP_MXWL, ", NP, NR, NSA, &
!                             RN_TEMP(NR,NS), RT_TEMP(NR,NS)
!                     END IF
                  ENDDO
               END IF
...
!            DO NR=1, NRMAX
!               WRITE(*,'(A,2I5,4E14.6)')&
!                    "TEST ", NR, NS, RT_READ(NR,NS), RN_READ(NR,NS), RT_TEMP(NR,NS), RN_TEMP(NR,NS)
!            END DO
!         END DO
!      END IF
...
!     Spitzer slowing down time on electron
      ...
      E_CR=14.8D0*PA(NS)/PA(NS)**(2.D0/3.D0)*RT_TEMP(NRSTART,1)*1.D3 ! assuming pure plasma
      zoa_eff=0.D0
      DO NS2=2, NSMAX
         zoa_eff = zoa_eff + RN_TEMP(NRSTART,NS2)*PZ(NS2)**2/( RN_TEMP(NRSTART,1)*PA(NS2) )
      END DO
!      A_b = 
      E_CR_2 =( 3.D0**(2.D0/3.D0)*PI**(1.D0/3.D0)*AMP**(1.D0/3.D0) )/ &
           (4.D0**(2.D0/3.D0)*AMFD(1)**(1.D0/3.D0))**PA(NS)*zoa_eff*RT_TEMP(NRSTART,1)
