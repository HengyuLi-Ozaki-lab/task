...
!>>>>>>> 99be190441bc8e2207d531603f711f4c3ea7e640
      ...
      real(rkind),dimension(:,:,:,:),POINTER :: & ! (NRM,NTG2M,NSAM,NSBM)
           RPCT2
      real(rkind),dimension(:,:),POINTER:: & !(NRM, NSAM)
           RN_TEMP, RT_TEMP, RN_READ, RT_READ, RN_BULK
...
!          deallocate(DCPP2B,DCPT2B,FCPP2B)
          ...
          IF(MODEL_EX_READ_Tn.ne.0)THEN
             ...
             IF(ASSOCIATED(read_tms_double)) DEALLOCATE(read_tms_double)
             IF(ASSOCIATED(read_cx_double)) DEALLOCATE(read_cx_double)
             IF(ASSOCIATED(read_tms_int)) DEALLOCATE(read_tms_int)
          END IF
