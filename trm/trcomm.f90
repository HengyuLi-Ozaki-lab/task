...
! TRSRC
  REAL(rkind):: &
       PNB_TOT,PNBIN_TOT,PNBCL_TOT
  REAL(rkind):: &
       PNF_TOT,PNFIN_TOT,PNFCL_TOT
  REAL(rkind):: &
       AJNB_TOT
  ...
  REAL(rkind), DIMENSION(:),     ALLOCATABLE :: & ! (NRM)
       PNBIN_NR,PNFIN_NR
  REAL(rkind), DIMENSION(:),     ALLOCATABLE :: & ! (NNXM)
       PNBIN_NNB,PNFIN_NNF
  REAL(rkind), DIMENSION(:,:),     ALLOCATABLE :: & ! (NNXM,NRM)
  ...
  REAL(rkind), DIMENSION(:,:),     ALLOCATABLE :: & ! (NNXM,NRM)
       PNBCL_NNBNR,PNFCL_NNFNR
  REAL(rkind), DIMENSION(:,:,:),     ALLOCATABLE :: & ! (NSM,NNXM,NRM)
...
! TRGLB
  ...
  REAL(rkind), DIMENSION(:)  ,ALLOCATABLE :: &  ! (NSTM)
       ANS0, TS0, ANSAV, ANLAV, TSAV, WST, PRFT, PBCLT, PFCLT, PLT, &
       SPET, SLT, pntot_ns
...
! TRINS
  ...
  SUBROUTINE allocate_trcomm(ierr)
    ...
    ALLOCATE(AJNB_NNBNR(NNBMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(AJNB_NNB(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ...
    ALLOCATE(PIN(NRMAX,NSTM),SSIN(NRMAX,NSTM),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PBCL(NRMAX,NSTM),PFCL(NRMAX,NSTM),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(SPE(NRMAX,NSTM),PRF(NRMAX,NSTM),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(SPE_NSNPELNR(NSMAX,NPELMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBIN_NNB(NNBMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBIN_NR(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBIN_NNBNR(NNBMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBCL_NS(NSMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBCL_NNB(NNBMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBCL_NR(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBCL_NSNR(NSMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBCL_NNBNR(NNBMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNBCL_NSNNBNR(NSMAX,NNBMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNFIN_NNF(NNFMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNFIN_NR(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNFIN_NNFNR(NNFMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNFCL_NS(NSMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNFCL_NNF(NNFMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNFCL_NR(NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNFCL_NSNR(NSMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
    ALLOCATE(PNFCL_NNFNR(NNFMAX,NRMAX),STAT=IERR)
      IF(IERR.NE.0) GOTO 900
 ...
 900 continue
  ...
  SUBROUTINE DEALLOCATE_TRCOMM
    ...
    DEALLOCATE(RNF,RTF)
    IF(ALLOCATED(ANF0)) DEALLOCATE(ANF0,TF0,ANFAV,TFAV)
    ...
    DEALLOCATE(SNF_NNF,PNF_NNF)
    DEALLOCATE(AJNB_NNBNR,AJNB_NNB)
    DEALLOCATE(PEC_NEC,PLH_NLH,PIC_NIC)
    DEALLOCATE(PNBIN_NNB,PNBIN_NR,PNBIN_NNBNR)
    DEALLOCATE(PNBCL_NS,PNBCL_NNB,PNBCL_NR)
    DEALLOCATE(PNBCl_NSNR,PNBCl_NNBNR)
    DEALLOCATE(PNBCL_NSNNBNR)
    DEALLOCATE(PNFIN_NNF,PNFIN_NR,PNFIN_NNFNR)
    DEALLOCATE(PNFCL_NS,PNFCL_NNF,PNFCL_NR)
    DEALLOCATE(PNFCL_NSNR,PNFCL_NNFNR)
    DEALLOCATE(PNFCL_NSNNFNR)
    DEALLOCATE(POH,PRB,PRC,PRL,PRSUM,PCX,PIE)
    DEALLOCATE(SIE,SCX,TSIE,TSCX,PIN,SSIN,PBCL,SPE,SPE_NSNPELNR)
    DEALLOCATE(PFCL,PRF,PRFV,AJRFV,RGFLX,SPSC)
    ...
    DEALLOCATE(SPSCT,PNBCLT,PNFCLT)
  ...
  SUBROUTINE DEALLOCATE_ERR_TRCOMM
    ...
    IF(ALLOCATED(ENF_NNF  ))     DEALLOCATE(ENF_NNF  )
    ...
    IF(ALLOCATED(PNBCLT   ))     DEALLOCATE(PNBCLT   )
    IF(ALLOCATED(PNFCLT   ))     DEALLOCATE(PNFCLT   )
