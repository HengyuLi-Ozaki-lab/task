### $Id$ ###
include make.header

all: message

message:
	@echo '## This is TASK main directory'
	@echo '     change working directory to'
	@echo '         pl, eq, tr, dp, wm, wr, fp or tot'
	@echo '     and make it'

clean:
	(cd adpost; make clean)
	(cd dp; make clean)
	(cd eq; make clean)
	(cd equ; make clean)
	(cd fit3d; make clean)
	(cd fp; make clean)
	(cd lib; make clean)
	(cd mtxp; make clean)
	(cd ob; make clean)
	(cd open-adas/adf11/adf11-lib; make clean)
	(cd pic; make clean)
	(cd pl; make clean)
	(cd t2; make clean)
	(cd ti; make clean)
	(cd tools; make clean)
	(cd tot; make clean)
	(cd tr; make clean)
	(cd trlib; make clean)
	(cd tx; make clean)
	(cd w1; make clean)
	(cd wf2; make clean)
	(cd wf2d; make clean)
	(cd wf3; make clean)
	(cd wf3d; make clean)
	(cd wi; make clean)
	(cd wim; make clean)
	(cd wiq; make clean)
	(cd wm; make clean)
	(cd wmf; make clean)
	(cd wq; make clean)
	(cd wr; make clean)

	(cd dpseki; make clean)
	(cd fp.anzai; make clean)
	(cd fp.nuga; make clean)
	(cd fp.ota; make clean)
	(cd fpx; make clean)
	(cd pt; make clean)
	(cd sak; make clean)
	(cd tf2d; make clean)
	(cd trn; make clean)
	(cd trx; make clean)
	(cd txnew; make clean)
	(cd wf2dt; make clean)
	(cd wf2dx; make clean)
	(cd wmfn; make clean)
	(cd wmseki; make clean)
	(cd wmx; make clean)
	(cd wrx; make clean)
	rm -f core a.out *.o ./*~

veryclean: clean
	(cd adpost; make veryclean)
	(cd dp; make veryclean)
	(cd eq; make veryclean)
	(cd equ; make veryclean)
	(cd fit3d; make veryclean)
	(cd fp; make veryclean)
	(cd lib; make veryclean)
	(cd mtxp; make veryclean)
	(cd ob; make veryclean)
	(cd open-adas/adf11/adf11-lib; make veryclean)
	(cd pic; make veryclean)
	(cd pl; make veryclean)
	(cd t2; make veryclean)
	(cd ti; make veryclean)
	(cd tools; make veryclean)
	(cd tot; make veryclean)
	(cd tr; make veryclean)
	(cd trlib; make veryclean)
	(cd tx; make veryclean)
	(cd w1; make veryclean)
	(cd wf2; make veryclean)
	(cd wf2d; make veryclean)
	(cd wf3; make veryclean)
	(cd wf3d; make veryclean)
	(cd wi; make veryclean)
	(cd wim; make veryclean)
	(cd wiq; make veryclean)
	(cd wm; make veryclean)
	(cd wmf; make veryclean)
	(cd wq; make veryclean)
	(cd wr; make veryclean)

	(cd dpseki; make veryclean)
	(cd fp.anzai; make veryclean)
	(cd fp.nuga; make veryclean)
	(cd fp.ota; make veryclean)
	(cd fpx; make veryclean)
	(cd pt; make veryclean)
	(cd sak; make veryclean)
	(cd tf2d; make veryclean)
	(cd trn; make veryclean)
	(cd trx; make veryclean)
	(cd txnew; make veryclean)
	(cd wf2dt; make veryclean)
	(cd wf2dx; make veryclean)
	(cd wmfn; make veryclean)
	(cd wmseki; make veryclean)
	(cd wmx; make veryclean)
	(cd wrx; make veryclean)

